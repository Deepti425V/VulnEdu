...
def _fetch_from_nvd_with_timeout(days=30, timeout=DEFAULT_TIMEOUT):
    ...
    try:
        result = _fetch_from_nvd(days, timeout)
        _record_success()
        return result
    except Exception as e:
        _record_failure()
...
def _fetch_from_nvd(days=30, timeout=DEFAULT_TIMEOUT):
    ...
    while call_count < max_calls:
        try:
            ...
            # Use the passed timeout parameter
            response = requests.get(
                NVD_API_URL,
                params=params,
                headers=headers,
                timeout=(5, timeout), # (connection timeout, read timeout)
                stream=False
            )
            response.raise_for_status()
            data = response.json()
        except requests.Timeout:
            print(f"[NVD API] Request {call_count + 1} timed out after {timeout} seconds")
            break
...
def _refresh_cache():
    ...
    try:
        ...
        cves = _fetch_from_nvd_with_timeout(days=30, timeout=30) # Fixed: now 30 days
        if not cves:
            print("[CVEs] No CVEs fetched from API, using fallback data")
            cves = _create_fallback_cves(days=30) # Fixed: now 30 days
        _save_to_cache(cves)
        ...
        fallback_cves = _create_fallback_cves(days=30) # Fixed: now 30 days
...
def get_all_cves(max_results=None, year=None, month=None, days=None, force_refresh=False, timeout=None):
    ...
    """
    # Fixed: Default to 30 days instead of 15
    if days is None:
        days = 30
    ...
    if not year and not month and not force_refresh:
    ...
    try:
        ...
        cves = _fetch_from_nvd_with_timeout(days=days, timeout=actual_timeout)
        if not cves:
