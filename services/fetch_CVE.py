...
# services/fetch_CVE.py (CHANGES MADE)
import requests
from config import NVD_API_URL, NVD_API_KEY
from datetime import datetime, timedelta, timezone
import os
...
CACHE_PATH = "data/cache/cve_cache.json"
CACHE_TIME_MINUTES = 1440  # refresh cache every 1 hour
...
def get_all_cves(max_results=None, year=None, month=None, days=None, force_refresh=False):
    """
    Loads CVEs from cache if available and fresh, else fetches from NVD
    """
    # Use cache only for the dashboard (no filters)
    if not year and not month and not days and not force_refresh and _is_cache_fresh():
        return _load_from_cache()
    now_utc = datetime.now(timezone.utc)
    all_cves = []
    ...
    while True:
        if days is not None and days > 0:
            ...
            pub_end = now_utc
            pub_start = pub_end - timedelta(days=29)
            params = {
                "resultsPerPage": results_per_page,
                "startIndex": start_index,
                "pubStartDate":
        ...
        start_index += results_per_page
    all_cves.sort(key=lambda x: x.get("Published") or "", reverse=True)
    # Only cache for full refresh (dashboard case: no filter)
    if not year and not month and not days:
        _save_to_cache(all_cves)
    return all_cves

def fetch_cve_from_nvd(cve_id):
    """Fetch a single CVE directly from NVD API"""
    headers = {"apikey": NVD_API_KEY}
    try:
        ...
        if not data.get('vulnerabilities'):
            return None
        item = data['vulnerabilities'][0].get('cve', {})
        published_str = item.get("published", "")
        description = next(
            (desc["value"] for desc in item.get("descriptions", []) if desc.get("lang") == "en"), ""
        )
        severity = "UNKNOWN"
        cvss = None
